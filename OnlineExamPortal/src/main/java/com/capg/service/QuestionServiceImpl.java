...
public class QuestionServiceImpl implements QuestionService {
	...
	public ResponseEntity<Question> getQuestionById(int q_id) throws QuestionNotFoundException {
		Question question = questionRepository.findById(q_id).orElseThrow(() -> new QuestionNotFoundException("Q_ID :: " + q_id + " Not Found"));
		return ResponseEntity.ok().body(question);
	}

	@Override
	public ResponseEntity<Question> updateQuestionById(int q_id, Question question) throws QuestionNotFoundException {
		Question newQuestion = questionRepository.findById(q_id).orElseThrow(() -> new QuestionNotFoundException("Q_ID:: " + q_id + " Not Found"));
